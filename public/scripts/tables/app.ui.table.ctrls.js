...
// Responsive Table Data (static)
        ...
        * */
        ...
        .controller("DataTablePizza", ["$scope", "$filter" , "$http",  function($scope, $filter, $http) {
            ...
            $http.get('http://localhost/foodis-restaurante/public/produto/restaurante/3').
                success(function(data, status, headers, config) {
                    $scope.produtos = data;
                    var prelength = $scope.produtos.length;
                    ...
                    $scope.search = function() {
                        $scope.filteredData = $filter("filter")($scope.produtos, $scope.searchKeywords);
                    ...
                    $scope.order = function(rowName) {
                        if($scope.row == rowName)
                            return;
                        $scope.row = rowName;
                        $scope.filteredData = $filter('orderBy')($scope.produtos, rowName);
                        $scope.onOrderChange();
                    }
